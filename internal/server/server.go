package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-parcelproxy/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-parcelproxy"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/packages",s.handleList)
    s.mux.HandleFunc("POST /api/packages",s.handleRecord)
    s.mux.HandleFunc("DELETE /api/packages/{id}",s.handleDelete)
